...
class User(AbstractUser):
    ...
    def active_profile(self):
        return self.profiles.filter(active=True).first()
...
class UserProfile(CommonModel):
    ...
    video = models.ForeignKey("users.UserVideo", null=True)
    active = models.BooleanField(default=False)

    @property
    def photos(self):
        photo_ids = ast.literal_eval(self.photo_ids)
        return UserPhoto.objects.filter(id__in=photo_ids)


class UserVideo(CommonModel):
    video = models.FileField(upload_to='uploads/videos/')
    thumbnail = models.FileField(upload_to='uploads/video_thumbnails/')


class UserPhoto(CommonModel):
    photo = models.FileField(upload_to='uploads/photos/')
    # profile = models.ForeignKey(UserProfile, related_name="medias")
